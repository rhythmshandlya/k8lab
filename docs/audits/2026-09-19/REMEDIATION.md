...
Production builds apply Drizzle migrations, including `0007_verified_problems.sql` and `0008_problem_drafts.sql`, before building the application. They require the direct `DATABASE_URL_UNPOOLED` connection, serialize concurrent production builds with an advisory lock, and stop deployment if migration fails. These additive migrations add verification/version markers and owner-scoped draft storage. Existing rows remain preserved as unverified history; they are deliberately excluded from verified XP, streaks and rankings. Problems move to content version 2. Older drafts can be exported or restored into the current workspace; older or imported solves require resubmission to verify.
...
Local database testing uses committed migrations on PGlite and PostgreSQL 16. Three concurrent production-migration runs against a fresh PostgreSQL database applied all nine migrations exactly once, and repeat migration and schema drift checks passed. Preview and local builds do not invoke the production migration gate. External OAuth/provider login and a production load test require deployment-specific configuration and are not represented as completed by these tests.
